...
void CSocket::ngx_event_accept(lpngx_connection_t oldc)
{
    ...
    do      // 用do，跳到while后边去会更方便
    {
        ...
        else
        {
            ...
            // 成功拿到连接池中一个连接
            memcpy(&newc->s_sockaddr, &mysockaddr, socklen);    // 拷贝客户端地址到连接对象【要转成字符串IP地址，参考函数ngx_sock_ntop()】
            ...
            newc->rhandler = &CSocket::ngx_read_request_handler;    // 设置数据来的时候的读处理函数。其实官方nginx中是ngx_http_wait_request_handler()
            newc->whandler = &CSocket::ngx_write_request_handler;   // 设置数据发送时的协处理函数
